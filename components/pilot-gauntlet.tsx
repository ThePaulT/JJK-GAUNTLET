...
import { PILOT_BOSSES, PILOT_ENCOUNTER_RULES, draftOptions, PILOT_PROFILES, PILOT_RULESET } from '@/lib/pilot-data.ts';
...
export function PilotGauntlet({ seed, onExit }: { seed: string; onExit: () => void }) {
  ...
  const options = draftOptions(picks);
  const round = run?.rounds[index];
  const done = run && !round;
  function pick(id: string) {
    if (run || picks.length >= 3 || !(options as readonly string[]).includes(id)) return;
  ...
  return <section className="flex flex-col gap-6">
    <header className="border-b border-sand pb-4"><p className="eyebrow">Hero Gauntlet · Curated draft</p><h1 className="display mt-2 text-3xl">{run ? 'Five bosses. One squad.' : `${picks.length + 1} / 3 · Choose your fighter`}</h1><p className="mt-2 max-w-2xl text-sm text-ash">{run ? 'Wounds and spent techniques carry forward. A retry continues the same fight.' : 'Build any trio from six fighters. Picked fighters leave the pool; combine the abilities you want.'}</p></header>
    {!run ? <>
      ...
      <p className="text-sm text-ash">{`${options.length} fighters available · ${3 - picks.length} picks left`}</p>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{options.map(id => <button key={id} onClick={() => pick(id)} type="button" className="panel cursor-pointer overflow-hidden text-left hover:border-bone focus-visible:outline-2 focus-visible:outline-curse">
